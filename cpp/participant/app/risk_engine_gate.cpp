...
oms::RiskDecision RiskEngineGate::decide(const events::exchange::NewOrderCommand& command) const {
  ...
  const risk::OmsDecision decision =
      engine_->decide_order(command.instrument_id, command.side, command.quantity_units,
                            command.client_order_id, clock_->now_utc());

  oms::RiskVerdict verdict = oms::RiskVerdict::kReject;
  switch (decision.verdict) {
    case risk::RiskVerdict::kApprove:
      verdict = oms::RiskVerdict::kApprove;
      break;
    case risk::RiskVerdict::kResize:
      verdict = oms::RiskVerdict::kResize;
      break;
    case risk::RiskVerdict::kReject:
      verdict = oms::RiskVerdict::kReject;
      break;
  }
