...
The far leg's *price* is not committed anywhere in ``data_samples`` (only one
contract's bar history exists per product in this repository) and is not
observed: it is constructed from the near leg's own observed price by a
fixed, documented, caller-supplied basis rule
(:class:`ConstructedBasisRule`), and every :class:`CalendarSpreadObservation`
this module returns names that rule's description explicitly. Nothing here
is presented as observed market data for the far leg (ADR-0025).
"""
...
def build_calendar_spread_observations(
    ...
    near_price_by_key = {(p.contract_id, p.session_date): p.price for p in near_prices}
    ...
    for index, as_of in enumerate(sorted(as_of_dates)):
        ...
        key = (near_id, as_of)
        if key not in near_price_by_key:
            raise CalendarSpreadDataError(
                f"no observed price for {near_id.canonical} on {as_of.isoformat()}"
            )
        near_price = near_price_by_key[key]
        ...
        observations.append(
            CalendarSpreadObservation(
                ...
                far_price_provenance=basis_rule.description,
