...
TEST(BookBuilder, TopOfBookReportsBestBidAskSpreadAndMid) {
  ...
  EXPECT_EQ(*top.best_bid, (PriceLevelView{100, 10}));
  EXPECT_EQ(*top.best_ask, (PriceLevelView{101, 15}));
  ASSERT_TRUE(top.spread_units.has_value());
  EXPECT_EQ(*top.spread_units, 1);
  ASSERT_TRUE(top.mid_price_units.has_value());
  EXPECT_DOUBLE_EQ(*top.mid_price_units, 100.5);
}
...
TEST(BookBuilder, MicropriceWeightsTowardTheOppositeSidesPrice) {
  ...
  EXPECT_DOUBLE_EQ(*price, (10.0 * 101.0 + 15.0 * 100.0) / 25.0);
}
...
TEST(BookBuilder, DepthImbalanceReflectsHeavierBidSide) {
  ...
  // (30 - 15) / (30 + 15) = 1/3.
  EXPECT_DOUBLE_EQ(*imbalance, (30.0 - 15.0) / (30.0 + 15.0));
}
...
TEST(BookBuilder, DepthImbalanceRespectsTheRequestedDepth) {
  ...
  EXPECT_DOUBLE_EQ(*top_only, (10.0 - 15.0) / (10.0 + 15.0));
}
...
TEST(BookBuilder, QueueDepletionTracksConsumptionAgainstTheHighWaterMark) {
  ...
  EXPECT_EQ(signal->original_quantity_units, 20);
  EXPECT_EQ(signal->current_quantity_units, 20);
  EXPECT_DOUBLE_EQ(signal->depletion_ratio, 0.0);
  ...
  ASSERT_TRUE(signal.has_value());
  EXPECT_EQ(signal->original_quantity_units, 20);  // High-water mark unchanged by a decrease.
  EXPECT_EQ(signal->current_quantity_units, 5);
  EXPECT_DOUBLE_EQ(signal->depletion_ratio, 1.0 - 5.0 / 20.0);
}
...
TEST(BookBuilder, AdverseSelectionDetectsAnUnfavorableMoveAfterTheWindow) {
  ...
  EXPECT_EQ(*resolved[0].mark_price_units, 90);
