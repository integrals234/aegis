...
def test_exempted_plan_reference_is_not_reported(tree):
    # experiments/plans/M1.md legitimately previews ADRs its own later slices
    ...
    (tree / "experiments/plans/M1.md").write_text(
        "Snapshot design is recorded in ADR-0013.\n", encoding="utf-8"
    )
    assert not any("ADR-0013" in e for e in run(tree))


def test_exemption_is_narrow_to_the_named_path(tree):
    ...
    assert any("references ADR-0013, which does not exist" in e for e in run(tree))


def test_exemption_is_narrow_to_the_named_numbers(tree):
    ...
    (tree / "experiments/plans").mkdir(parents=True)
    (tree / "experiments/plans/M1.md").write_text(
        "Unrelated future work is recorded in ADR-0099.\n", encoding="utf-8"
    )
